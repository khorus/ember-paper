...
export default Ember.Mixin.create({
  ...
  didInsertElement() {
    ...
    if (!this.get("noink")) {
      this.jqueryElement = this.$();
      this.colorElement = this.$();
      this.node = this.$()[0];
      this.hammertime = propagating(new Hammer(this.node));
      this.color =
        this.parseColor(this.jqueryElement.attr("md-ink-ripple")) ||
        this.parseColor(
          window.getComputedStyle(this.colorElement[0]).color || "rgb(0, 0, 0)"
        );
  ...
  getRippleElement(css) {
    var elem = Ember.$(
      '<div class="md-ripple" data-counter="' + this.counter++ + '">'
    );
  ...
  getRippleCss(size, left, top) {
    ...
    if (this.get("center")) {
      ...
      css.left =
        Math.round(
          ((left - rect.left) / this.rippleContainer.prop("offsetWidth")) * 100
        ) + "%";
      css.top =
        Math.round(
          ((top - rect.top) / this.rippleContainer.prop("offsetHeight")) * 100
        ) + "%";
    }
